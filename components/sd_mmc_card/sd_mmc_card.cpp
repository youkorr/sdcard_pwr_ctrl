...
void SdMmc::setup() {
  ...
  if (this->power_ctrl_pin_ >= 0) {
    ...
    // Active l'alimentation de la carte SD
    gpio_set_level(static_cast<gpio_num_t>(this->power_ctrl_pin_), 0);  // Remarque : parfois c'est HIGH (1) pour activer, donc vérifie le fonctionnement sur ta carte
    ESP_LOGI(TAG, "SD Card power control enabled on GPIO %d", this->power_ctrl_pin_);

    // Ajouter un délai pour la stabilisation de l'alimentation (important pour certaines cartes SD)
    vTaskDelay(pdMS_TO_TICKS(500));  // Délai de 500ms, ajuste si nécessaire
  }

  // Configuration pour le montage de la carte SD
  ...
  // Configuration des GPIOs pour les pins SD
  ...
  // Vérification du mode (1-bit ou 4-bit)
  host.flags = mode_1bit_ ? SDMMC_HOST_FLAG_1BIT : 0;
  ESP_LOGI(TAG, "Using %s mode", mode_1bit_ ? "1-bit" : "4-bit");
